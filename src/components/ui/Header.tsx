...
export default function Header() {
    ...
    return (
        <>
            <header className="fixed top-0 z-50 w-full p-6 md:p-8 mix-blend-difference text-white pointer-events-none">
            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    <motion.div
                        ...
                        className="fixed inset-0 bg-neutral-950 text-white z-40 flex flex-col pt-32 px-6 pb-12 overflow-y-auto mix-blend-normal pointer-events-auto"
                        ...
                        <nav className="flex flex-col gap-6 text-4xl sm:text-5xl font-bold tracking-tighter uppercase mb-auto border-t border-neutral-800 pt-12">
                            {links.map((link) => (
                                <Link
                                    key={link.name}
                                    href={link.href}
                                    className="flex py-2 w-full hover:opacity-75 active:text-neutral-400 transition-colors"
